<script lang="ts">
    ...
    // import { initializeAuthPersistence, onAuthStateChanged } from '../../lib/auth';
    ...
    onMount(async () => {
        try {
            ...
            // Add click listener for handling outside clicks
    ...
    async function handleCreateList() {
        ...
        try {
            ...
            const newList = await addList({ name: newListName, ownerId: currentUserId, sharedBy: newSharedWith });
            if (newList) {
                lists = [...lists, newList];
