...
function pinch(node, inputParameters) {
  ...
  function onMove(activeEvents, event) {
    if (activeEvents.length === 2) {
      ...
      if (prevDistance !== void 0 && curDistance !== prevDistance) {
...
function rotate(node, inputParameters) {
  ...
  function onMove(activeEvents, event) {
    if (activeEvents.length === 2) {
      ...
      if (prevAngle !== void 0 && curAngle !== prevAngle) {
...
function callAllByType(ListenerType, subGestureFunctions, activeEvents, event) {
  subGestureFunctions.forEach((gesture) => {
    var _a;
    (_a = gesture[ListenerType]) == null ? void 0 : _a.call(gesture, activeEvents, event);
  });
}
...
var DEFAULT_TRESHOLD = 0.9;
var DEFAULT_NB_OF_SAMPLE_POINTS = 64;
var PHI = (Math.sqrt(5) - 1) / 2;
var ANGLE_RANGE_RAD = deg2Rad(45);
var ANGLE_PRECISION_RAD = deg2Rad(2);
function deg2Rad(d) {
...
function distanceAtBestAngle(pattern, points, center) {
  ...
  let angleOne = PHI * fromAngleRad + (1 - PHI) * toAngleRad;
  let distanceOne = distanceAtAngle(pattern, angleOne, points, center);
  let angleTwo = (1 - PHI) * fromAngleRad + PHI * toAngleRad;
...
function shapeDetector(inputPatterns, options = {}) {
  ...
  function getStroke(points, name, allowRotation) {
    ...
    function translateToOrigin(center2) {
      return points.map((point) => ({
        x: point.x - center2.x,
        y: point.y - center2.y
      }));
    ...
    function indicativeAngle(center2) {
      return Math.atan2(center2.y - points[0].y, center2.x - points[0].x);
    }
  ...
  function detect(points, patternName = "") {
    ...
    patterns.forEach((pattern) => {
      if (pattern.name.indexOf(patternName) > -1) {
        ...
        const score = 1 - distance / HALF_SQUARE_DIAGONAL;
